...
(use-package sqlformat
  ...
  (setq sqlformat-command 'pgformatter
        sqlformat-args '("-s2" "-g")))
#+END_SRC
*** WIP - Emacs Everywhere
This is neat for calling an Emacs buffer whenever you need to do something away
from your main Emacs window.
You'll need to add a keybinding to the window manager you're using to invoke
~emacsclient~ like so:
#+BEGIN_SRC shell
...
(cond
 ...
 ((eq system-type 'gnu/linux)
  (if (font-info "Terminus")
      (set-face-attribute 'default nil :font "-xos4-Terminus-normal-normal-normal-*-18-*-*-*-c-100-iso10646-1" :height 100)
...
(setq display-buffer-alist
      ...
      `(("^magit: .*$"
        ...
        ("\\(^\\*ag search text:.*\\*$\\|\\^*Occur.*\\*$\\)"
         ...
         (inhibit-same-window . t))
        ;; This is so I can close other windows without switching to them.
        ("^.*\\.ex[s]?$"

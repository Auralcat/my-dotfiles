...
#+END_SRC
*** Indent-tools
Kinda necessary for working in a sane way with YAML files if you don't use ~evil-mode~.
#+BEGIN_SRC emacs-lisp :tangle yes
...
** Personal keymap
Since I'm planning to drop ~evil-mode~ recently, I'll need to transfer what keeps me using it: the keybindings I have on ~evil-leader~. I need them to call quick functions I use often, so we can transfer them to a personal keymap and bind the keymap prefix to a key.

#+BEGIN_SRC emacs-lisp :tangle yes
(defvar auralcat-map
  (let ((map (make-sparse-keymap)))
    ;; Your keybindings go here.
    (define-key map (kbd "fo") 'my-find-init-org-file)
    (define-key map (kbd "fe") 'my-find-evilrc-file)
    (define-key map (kbd "fs") 'jump-to-scratch-buffer)
    (define-key map (kbd "fm") 'jump-to-messages-buffer)
    (define-key map (kbd "j") 'jump-to-register)
    (define-key map (kbd "p") 'projectile-command-map)
    (define-key map (kbd "k") 'auralcat/quit-and-kill-buffer)
        map)
  "My personal keymap.")

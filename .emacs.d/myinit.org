...
Saves you from getting lost in indentations.
#+BEGIN_SRC emacs-lisp :tangle yes
...
*** Format all the code
There is a package to run formatters on your source code. I used to
have a custom solution for this, but I think using a package for it is
more maintainable and works better.
...
* SysAdmin things
** ESS
Emacs Speaks Shell is a package to turn the ~shell-mode~ buffer in a REPL and send shell commadns from one buffer to there!
#+begin_src emacs-lisp :tangle yes
(require 'essh)
(defun essh-sh-hook ()
  (define-key sh-mode-map "\C-c\C-r" 'pipe-region-to-shell)
  (define-key sh-mode-map "\C-c\C-b" 'pipe-buffer-to-shell)
  (define-key sh-mode-map "\C-x\C-e" 'pipe-line-to-shell)
...
Taken from https://www.emacswiki.org/emacs/CfnLint
#+begin_src emacs-lisp :tangle yes
...
** Utilities
*** Engine-mode
Make web searches from the comfort of your Emacs.
#+BEGIN_SRC emacs-lisp :tangle yes
...
(dolist (major-mode '(org-english-mode org-mode))
 ...
 'append))
#+END_SRC
** Kill entire heading with a single command
#+BEGIN_SRC emacs-lisp :tangle yes
(defalias 'auralcat/org-kill-heading
   (kmacro "C-c @ C-w"))

(define-key org-mode-map (kbd "C-S-k") 'auralcat/org-kill-heading)
#+END_SRC
...
(use-package howm
  ...
  ;; In this case I'm using .txt to open these files in text-mode.
  (setq howm-file-name-format "%Y-%m-%d__%H%M%S.org.en")
  ;; Changing the prefix because it clashes with org-mode and rspec-mode.
...
(defun auralcat/calendar-last-weekday-of-month (date)
...
These are meant to reduce verbosity when scheduling events in org-mode.
#+begin_src emacs-lisp :tangle yes
(defun auralcat/calendar-weekday (date)
  "Return whether DATE is a weekday or not."
  (let ((weekdays '(1 2 3 4 5)))
    (memq (calendar-day-of-week date) weekdays)))

(defun auralcat/calendar-tuesday (date)
  "Return whether DATE is a Tuesday.
This is represented by the day code 2 in `calendar-day-of-week"
  (let ((tuesday-code '(2)))
    (memq (calendar-day-of-week date) tuesday-code)))

(defun auralcat/calendar-wednesday (date)
  "Return whether DATE is a Wednesday.
This is represented by the day code 3 in `calendar-day-of-week"
  (let ((wednesday-code '(3)))
    (memq (calendar-day-of-week date) wednesday-code)))
#+end_src

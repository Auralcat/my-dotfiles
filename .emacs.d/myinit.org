...
;; Calendar location settings (required for Circadian theme switching)
;; These coordinates are for a general location - adjust for your specific area
(setopt calendar-latitude 40.7128    ; New York City latitude (adjust as needed)
        calendar-longitude -74.0060  ; New York City longitude (adjust as needed)
        calendar-location-name "Your City, Country") ; Customize this

;; Alternative: Get location automatically (requires 'request' package)
;; (setopt calendar-latitude nil
...
(use-package gcmh
  :demand t  ;; Need this early for startup performance
...
(use-package markdown-mode
  ...
  :bind (:map markdown-mode-map
         ...
         ("C-c M-w" . au/markdown-copy-block-content))
...
(use-package diminish
  :demand t  ;; Need this early to diminish built-in modes
  :config
  ;; Diminish built-in modes
  (diminish 'auto-revert-mode)
  (diminish 'auto-fill-mode)
  (diminish 'abbrev-mode)
  ;; Diminish modes loaded by other packages
...
(use-package yasnippet
  ...
  (yas-reload-all))
    #+END_SRC
*** System Appearance (freedesktop portal)
    Switches themes automatically based on system dark/light mode via built-in dbus.
    Works on Linux with GNOME/KDE/any desktop that supports the freedesktop portal.
    #+BEGIN_SRC emacs-lisp :tangle yes
(defun auralcat/apply-system-appearance ()
  "Apply theme based on current system appearance via freedesktop portal."
  (let* ((color-scheme
          (ignore-errors
            (caar (dbus-call-method
                   ...
                   "color-scheme"))))
         (dark-p (equal color-scheme 1)))
    (auralcat/change-theme (if dark-p 'ef-dream 'ef-day))))

(defun auralcat/on-appearance-changed (namespace key value)
  "Handle system appearance change signal from freedesktop portal."
  (when (and (equal namespace "org.freedesktop.appearance")
             (equal key "color-scheme"))
    (let ((dark-p (equal (car value) 1)))
      (auralcat/change-theme (if dark-p 'ef-dream 'ef-day)))))

;; Register dbus signal to react to system theme changes
(dbus-register-signal
 ...
 "org.freedesktop.portal.Settings"
 "SettingChanged"
 #'auralcat/on-appearance-changed)

;; Apply theme matching system appearance on startup
(add-hook 'after-init-hook #'auralcat/apply-system-appearance)
    #+END_SRC
...
#+END_SRC
*** Emacs MCP server
Control Emacs with LLMs!
#+begin_src emacs-lisp :tangle yes
(use-package mcp-server
  :load-path "~/emacs-mcp-server"
  :config
  (add-hook 'emacs-startup-hook #'mcp-server-start-unix))

(setq mcp-server-security-allowed-dangerous-functions
        '(find-file-noselect with-current-buffer save-buffer))
#+end_src
*** TMR
A tiny timer inside your Emacs. Quite useful for ADHD brains like mine!
#+begin_src emacs-lisp :tangle yes
(use-package tmr
  :hook (after-init . tmr-mode-line-mode)
  :bind-keymap ("C-c t" . tmr-prefix-map)
  :custom (tmr-notification-urgency 'critical))
#+end_src
...
;; Optimize font-lock for large files
(setopt font-lock-maximum-decoration 2      ; Balance between speed and highlighting
        font-lock-maximum-size (* 1024 1024) ; 1MB limit
        jit-lock-stealth-time 16             ; Slower background highlighting
        jit-lock-defer-time 0.05             ; Small delay before highlighting
        jit-lock-chunk-size 1024)            ; Smaller chunks

;; Add mode-specific optimizations
(add-hook 'prog-mode-hook
          (lambda ()
            (when (> (buffer-size) (* 100 1024)) ; Files > 100KB
...
(use-package indent-bars
  ...
  :custom
  ;; Disable current depth highlighting to prevent visual shifting
  (indent-bars-highlight-current-depth nil)
  ;; Disable current depth coloring for consistency
  (indent-bars-color-by-depth nil)
  ;; Keep bars stable and consistent
  (indent-bars-prefer-character t)
  ;; Additional settings to prevent any dynamic behavior
  (indent-bars-display-on-blank-lines t)
  ;; Ensure consistent positioning
  (indent-bars-starting-column 0)
  ;; Use subtle, consistent visual style
  (indent-bars-width-frac 0.1)
  (indent-bars-pad-frac 0.1)
  ;; Disable scope-based adjustments that can cause shifting
...
(use-package vterm
  ...
  :bind (:map vterm-mode-map
         ...
         ("C-c C-y" . vterm-yank)
         ("©" . vterm-copy-mode)          ;; AltGr+c
         ("„" . vterm-yank)               ;; AltGr+v
         :map vterm-copy-mode-map
         ("C-c C-w" . kill-ring-save)
         ("C-w" . kill-region)
         ("M-w" . kill-ring-save))
  :config
  ;; Make cursor visible in copy mode
...
** Main configuration
   #+BEGIN_SRC emacs-lisp :tangle yes
     ...
     (setopt org-hierarchical-todo-statistics nil
       ...
       org-todo-keywords '((sequence "TODO" "WAITING" "SOMEDAY" "BLOCKED" "|" "DONE" "CANCELED"))
       org-agenda-scheduled-leaders '("Scheduled: " "Sched. previously %2dx: ")
       org-agenda-skip-scheduled-if-done t
       org-agenda-start-on-weekday 0
       org-agenda-files '("/home/mretka/file-bouncer/org-files/vida/"))
     ...
     ;; Defer keybinding setup until org-mode is loaded
...
** Org-habit
   Configuração do org-habit para rastrear hábitos recorrentes.

   #+BEGIN_SRC emacs-lisp :tangle yes
     ;; Carregar o módulo org-habit
     (require 'org-habit)
     (add-to-list 'org-modules 'org-habit)

     ;; Configurações de exibição do org-habit
     (setopt org-habit-show-habits-only-for-today nil  ;; Mostrar hábitos futuros
             org-habit-graph-column 60                  ;; Posição do gráfico na agenda
             org-habit-preceding-days 21                ;; Dias anteriores no gráfico
             org-habit-following-days 7)                ;; Dias seguintes no gráfico
   #+END_SRC
...
* Reading-focused settings
Better outline visibility settings for reading Org and Markdown files.
#+BEGIN_SRC emacs-lisp :tangle yes
(setopt org-startup-folded 'content)
(setopt org-hide-leading-stars t)
(setopt org-odd-levels-only nil)
(setopt org-cycle-separator-lines 2)

;; Simple navigation keys for view-mode (when reading)
(defun setup-view-mode-navigation ()
  "Setup simple navigation keys for reading structured files in view-mode."
  (when (bound-and-true-p view-mode)
    (cond
     ;; Org-mode navigation
     ((derived-mode-p 'org-mode)
      (define-key view-mode-map (kbd "j") 'org-next-visible-heading)
      (define-key view-mode-map (kbd "k") 'org-previous-visible-heading)
      (define-key view-mode-map (kbd "h") 'outline-up-heading)
      (define-key view-mode-map (kbd "l") 'org-forward-heading-same-level)
      (define-key view-mode-map (kbd "g") 'org-goto)
      (define-key view-mode-map (kbd "/") 'org-sparse-tree)
      (define-key view-mode-map (kbd "TAB") 'org-cycle)
      (define-key view-mode-map (kbd "S-TAB") 'org-global-cycle))
     ;; Markdown-mode navigation
     ((derived-mode-p 'markdown-mode)
      (define-key view-mode-map (kbd "j") 'markdown-next-heading)
      (define-key view-mode-map (kbd "k") 'markdown-previous-heading)
      (define-key view-mode-map (kbd "h") 'markdown-up-heading)
      (define-key view-mode-map (kbd "l") 'markdown-forward-same-level)
      (define-key view-mode-map (kbd "g") 'markdown-goto-heading)
      (define-key view-mode-map (kbd "/") 'isearch-forward)
      (define-key view-mode-map (kbd "TAB") 'markdown-cycle)
      (define-key view-mode-map (kbd "S-TAB") 'markdown-shifttab)))))

(add-hook 'view-mode-hook 'setup-view-mode-navigation)
#+END_SRC
...
;; Update fringe background whenever the theme changes.
(add-hook 'enable-theme-functions (lambda (_theme) (auralcat/set-fringe-face-to-default-bg)))
...
(use-package ef-themes
  ...
  :custom
  ;; Set theme preferences using :custom for proper type checking
...
** Modus Themes
Built-in themes from Protesilaos Stavrou, excellent for accessibility.
#+begin_src emacs-lisp :tangle yes
(use-package modus-themes
  :demand t  ; Load immediately
  :custom
  ;; Configure theme preferences using :custom for proper type checking
...
(add-hook 'enable-theme-functions #'(lambda (theme) (auralcat--set-preferred-fonts-for-current-theme)))

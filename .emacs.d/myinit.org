...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (add-to-list 'load-path "~/my-dotfiles/.emacs.d/manual-packages/multi-term-plus/")
...
*** Multi-term Plus
    A newer version of Multi Term maintained by a person from Japan.
    #+BEGIN_SRC emacs-lisp :tangle yes
...
*** Carbon-now.sh
    ...
    #+END_SRC
*** Linum-relative
    Show relative line numbers. Stacks with linum or
    display-line-numbers-mode, so disable it before using this one.
    #+BEGIN_SRC emacs-lisp :tangle yes
...
** Gruvbox
   ...
   # light/dark mode switching in the way of solarized. The main focus when
   # developing gruvbox is to keep colors easily distinguishable, contrast
   # enough and still pleasant for the eyes.
   # #+BEGIN_SRC emacs-lisp :tangle yes

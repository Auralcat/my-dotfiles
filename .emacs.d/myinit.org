...
(use-package yasnippet
  ...
  (yas-reload-all))
    #+END_SRC
*** Circadian
    Theme changer for Emacs. Automatically switches themes based on sunrise/sunset.
    #+BEGIN_SRC emacs-lisp :tangle yes
(use-package circadian
  :after (ef-themes modus-themes)  ; Ensure themes are loaded first
  :demand t  ; Still load immediately, but after dependencies
  :config
  ;; Validate that required calendar variables are set
  (unless (and calendar-latitude calendar-longitude)
    (user-error "Calendar location not configured! Please set calendar-latitude and calendar-longitude."))
  ...
  ;; Validate that the configured themes exist before setup
  (let ((themes-to-check (mapcar #'cdr (symbol-value 'circadian-themes))))
    (dolist (theme themes-to-check)
      (unless (or (eq theme 'user)  ; Skip 'user' theme
                  (custom-theme-p theme)
                  (locate-file (concat (symbol-name theme) "-theme.el")
                               (custom-theme--load-path) '("" ".el")))
        (warn "Circadian: Theme '%s' not found, but continuing setup..." theme))))

  ;; Now safely setup Circadian
  (condition-case err
      (circadian-setup)
    (error
     (message "Circadian setup failed: %s" (error-message-string err))
     (message "Falling back to manual theme switching with M-` key"))))
    #+END_SRC
...
(use-package ef-themes
  :demand t  ; Load immediately for Circadian
  :custom

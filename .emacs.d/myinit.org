...
(defun auralcat/elixir-run-mix-format-from-project-root ()
  "Runs 'mix format' from the project's root over the current buffer's file."
  (let* (
        (current-buffer-directory (cadr (split-string (pwd))))
        (project-root-directory (expand-file-name (vc-root-dir))))
    (with-current-buffer (current-buffer)
      (when (eq major-mode 'elixir-mode)
        (unless (string-equal default-directory project-root-directory)
          (cd project-root-directory))

        (shell-command (string-join (list "mix" "format" (buffer-file-name)) " "))
        (cd current-buffer-directory)))))

;; Need to try this out in the work computer.
;; (add-hook 'after-save-hook 'auralcat/elixir-run-mix-format-from-project-root)
